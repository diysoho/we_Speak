...
onReady(){
  ...
  wx.onSocketOpen(function (res) {
    ...
    if (that.data.room) {
      ...
      var creat = app.globalData.openid;
      var openid = app.globalData.openid;
      var us = wx.getStorageSync("usermg");
      var id = us.id;
      var str = room_id + openid + id;
      var jm = util.hexMD5(str);
      var tjm = jm + appid;
      var sig = util.hexMD5(tjm);
      that.setData({
        roomid: room_id
      })
      wx.request({
        ...
        data: {
          room_id: room_id,
          create_user_openid: creat,
          openid: app.globalData.openid
        },
        success: res => {
          wx.hideLoading();
          sendSocketMessage('{ "type": "login", "room_id":"' + room_id + '","openid":"' + openid + '", "id":"' + id + '","sign":"' + sig + '"}')
          if (res.data.code == 0) {
            var headpic = res.data.data.user.headpic;
            var nick = getApp().globalData.userInfo.nickName;
            var total_money = res.data.data.room.red_money;
            var total_num = res.data.data.room.red_num;
            var kouling = res.data.data.room.kouling;
            that.setData({
  ...
  wx.onSocketMessage(function (res) {
    ...
    switch (data.type) {
      ...
      case 'logined':
        ...
        wx.request({
          url: app.globalData.url + '/Bns/Room/getCurRoomList?client_id=' + client_id + '&room_id=' + app.globalData.room_id,
          success: function (res) {
          },
          fail: function (res) {
          }
  ...
  gotoPlay: function (e) {
    ...
    if (this.idnexP == e.currentTarget.dataset.index && this.play) {
      ...
      wx.downloadFile({
        url: "https://www.fansba.com.cn/Bns" + filePath, //服务器文件
        success: function (res) {
          wx.playVoice({
            filePath: res.tempFilePath,
            complete: function () {
